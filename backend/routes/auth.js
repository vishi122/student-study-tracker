...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        let user = null;
        try {
            user = await User.create({ name, email, password: hashedPassword }).catch(err => null);
        } catch (e) {
            user = null;
        }
        ...
        res.status(201).json({ _id: id, name: user.name, email: user.email, token: generateToken(id) });
    } catch (error) {
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (user && (user.matchPassword ? await user.matchPassword(password) : await bcrypt.compare(password, user.password))) {
            res.json({ _id: user._id || user.id, name: user.name, email: user.email, token: generateToken(user._id || user.id) });
        } else {
