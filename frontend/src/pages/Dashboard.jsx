...
const Dashboard = () => {
    ...
    if (loading) return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-pulse">
            {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-32 glass-card bg-slate-800/50"></div>
            ))}
        </div>
    );
    ...
    return (
        <div className="space-y-8 pb-10">
            <header>
                ...
                <p className="text-slate-400 mt-1">Track your progress and study habits.</p>
            </header>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {stats.map((stat) => (
                    <div
                        key={stat.label}
                        className="glass-card p-6 flex items-center justify-between group hover:border-primary-500/50 transition-all cursor-default"
                    >
                        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 glass-card p-6">
                    <h3 className="text-lg font-semibold text-white mb-6">Weekly Activity (Duration)</h3>
                        ...
                        {analytics?.insights?.length > 0 ? (
                            (showAllInsights ? analytics.insights : analytics.insights.slice(0, 3)).map(
                                (insight, idx) => (
                                    <div
                                        key={idx}
                                        className="p-4 rounded-lg bg-primary-600/10 border border-primary-600/20 flex gap-3"
                                    >
                    ...
                    <div className="pt-4 border-t border-slate-800">
                        ...
                        <div className="space-y-3">
                            {analytics?.subjectStats && Object.keys(analytics.subjectStats).length > 0 ? (
                                ...
                                <p className="text-slate-500 text-xs italic">No data available yet.</p>
                            )}
                        </div>
                    </div>
                </div>
